...
All terminating-decimal strings are converted to exact Fractions before any
multiplication/subtraction.  This deliberately avoids Decimal's finite context
precision, which can otherwise make an exact transcription appear unequal.
"""
...
for line in lines[3:]:
    ...
    if a[0] == 'COS':
        ...
        raise SystemExit(f'bad tag {a[0]}')

expected_cos = [(r['xi'], r['lambda']) for r in d['cosine_rows']]
wl = rat(d['window_lambda'])
weights = [rat(x) for x in d['window_weights']]
h = {int(r['n']): rat(r['lambda']) for r in d['harmonic_rows']}
expected_pi = {
    n: wl * weights[n - 1] - h.get(n, Fraction(0))
